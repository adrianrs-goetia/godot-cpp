...
def options(opts, env):
    ...
    opts.Add(BoolVariable("godot_cpp_unity_build", "Generate unity builds for godot-cpp", True))
...
def _generate_unity_sources(env, sources: list, batch_size: int):
    from pathlib import Path
    import math
    import shutil

    unity_dir = Path("obj/src")
    obj_dir = Path("obj")
    # remove existing generated files
    shutil.rmtree(unity_dir.as_posix(), ignore_errors=True)
    unity_dir.mkdir(parents=True, exist_ok=True)
    obj_dir.mkdir(parents=True, exist_ok=True)

    # Generate .cpp unity files
    cwd = os.getcwd()
    unity_files = []
    ...
    for f in unity_files:
        obj_target = os.path.join(obj_dir, os.path.relpath(f) + ".o")
        objs.append(env.Object(target=obj_target, source=f))
    return objs
...
def _godot_cpp(env):
    ...
    sources.extend([str(f) for f in bindings if str(f).endswith(".cpp")])
    if env["godot_cpp_unity_build"]:
        print("Unity build of godot-cpp")
        sources = _generate_unity_sources(env, sources, batch_size=20)
    else:
        print("Regular build of godot-cpp")
